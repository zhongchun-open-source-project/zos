...
#ifndef SM_BASE_METRICS_H_
#define SM_BASE_METRICS_H_
...
//   Histogram* metrics::HistogramFactoryGetCounts(
//       const std::string& name, int sample, int min, int max,
//       int bucket_count);
//   Histogram* metrics::HistogramFactoryGetEnumeration(
//       const std::string& name, int sample, int boundary);
//   void metrics::HistogramAdd(
...
#define RTC_HISTOGRAM_COUNTS(name, sample, min, max, bucket_count) \
  RTC_HISTOGRAM_COMMON_BLOCK(name, sample, \
      metrics::HistogramFactoryGetCounts(name, min, max, bucket_count))
...
#define RTC_HISTOGRAM_ENUMERATION(name, sample, boundary) \
  RTC_HISTOGRAM_COMMON_BLOCK(name, sample, \
      metrics::HistogramFactoryGetEnumeration(name, boundary))
...
#define RTC_HISTOGRAM_COMMON_BLOCK(constant_name, sample, \
  ...
  do { \
    static metrics::Histogram* atomic_histogram_pointer = nullptr; \
    metrics::Histogram* histogram_pointer = \
        AtomicOps::AcquireLoadPtr(&atomic_histogram_pointer); \
    if (!histogram_pointer) { \
      histogram_pointer = factory_get_invocation; \
      metrics::Histogram* prev_pointer = \
          AtomicOps::CompareAndSwapPtr( \
              &atomic_histogram_pointer, \
              static_cast<metrics::Histogram*>(nullptr), \
              histogram_pointer); \
      RTC_DCHECK(prev_pointer == nullptr || \
                 prev_pointer == histogram_pointer); \
    } \
    metrics::HistogramAdd(histogram_pointer, constant_name, sample); \
  } while (0)
...
#define RTC_HISTOGRAM_COMMON_BLOCK_SLOW(name, sample, factory_get_invocation) \
  do { \
    metrics::Histogram* histogram_pointer = factory_get_invocation; \
    metrics::HistogramAdd(histogram_pointer, name, sample); \
  } while (0)
