...
DEFINE_string(
    ...
    "E.g. running with --force_fieldtrials=SM-FooFeature/Enable/"
    " will assign the group Enable to field trial SM-FooFeature.");
#if defined(SM_WIN)
...
int main(int argc, char** argv) {
  ...
  test::InitFieldTrialsFromString(FLAG_force_fieldtrials);
  ...
  Filesystem::SetOrganizationName("google");
  Filesystem::SetApplicationName("unittest");

  // By default, log timestamps. Allow overrides by used of a --log flag.
  LogMessage::LogTimestamps();
  if (*FLAG_log != '\0') {
    LogMessage::ConfigureLogging(FLAG_log);
  } else if (LogMessage::GetLogToDebug() > LS_INFO) {
