...
//     error. errno and associated facilities exist on both Win32 and POSIX,
//     but on Win32 they only apply to the C/C++ runtime.
// LOG_ERR(sev) is an alias for the platform's normal error system, i.e. _GLE on
//     Win32 and _ERRNO on POSIX.
...
#ifndef SM_BASE_LOGGING_H_
#define SM_BASE_LOGGING_H_
...
class LogMessage {
 public:
  ...
  //  Debug: Debug console on Win32, otherwise stderr
 ...
 private:
  ...
  SM_DISALLOW_COPY_AND_ASSIGN(LogMessage);
};
...
#define LOG_TAG(sev, tag) \
  LOG_SEVERITY_PRECONDITION(sev) \
    LogMessage(NULL, 0, sev, tag).stream()
