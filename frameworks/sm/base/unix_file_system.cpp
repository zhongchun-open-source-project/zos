...
#if defined(SM_POSIX) && !defined(SM_MAC) || defined(SM_IOS)
#include <sys/types.h>
...
void IOSAppName(Pathname* path);
#endif

namespace sm {

#if !defined(SM_ANDROID) && !defined(SM_IOS)
...
#endif  // !SM_MAC || SM_IOS && !SM_ANDROID
  if (append) {
    ASSERT(!append->empty());
...
#elif defined(SM_ANDROID) || defined(SM_IOS)  // && !SM_MAC || SM_IOS
  ASSERT(provided_app_data_folder_ != NULL);
  path->SetPathname(provided_app_data_folder_, "");
#elif defined(SM_LINUX) && !defined(SM_ANDROID)  // && !SM_MAC && !SM_IOS && !SM_ANDROID
  if (per_user) {
...
#endif  // !SM_MAC && !SM_LINUX

  // Now add on a sub-path for our app.
#if defined(SM_MAC) || defined(SM_ANDROID)
