...
// Try to catch any problem with scoped_refptr type deduction in Bind at
// compile time.
static_assert(
    is_same<
        remove_reference<const scoped_refptr<RefCountInterface>&>::type,
        const scoped_refptr<RefCountInterface>>::value,
    "const scoped_refptr& should be captured by value");

static_assert(is_same<remove_reference<const scoped_refptr<F>&>::type,
                      const scoped_refptr<F>>::value,
              "const scoped_refptr& should be captured by value");

static_assert(
    is_same<remove_reference<const int&>::type, const int>::value,
    "const int& should be captured as const int");

static_assert(is_same<remove_reference<const F&>::type, const F>::value,
              "const F& should be captured as const F");

static_assert(is_same<remove_reference<F&>::type, F>::value,
