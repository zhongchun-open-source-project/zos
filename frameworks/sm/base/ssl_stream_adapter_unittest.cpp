...
#include "sm/base/buffer_queue.h"
...
static const char kRSA_PRIVATE_KEY_PEM[] =
    ...
    "czsZ+6JDhDvnkF+vn6yCAGuRPV03zuRqZtDy4N4to7PZu9PjqrRl7nDMSmG3YG9y\n"
...
class SSLDummyStreamBase : public StreamInterface,
 ...
 public:
  ...
  void OnEventIn(StreamInterface* stream, int sig, int err) {
  ...
  void OnEventOut(StreamInterface* stream, int sig, int err) {
    if (sig & SE_WRITE) {
      LOG(LS_INFO) << "SSLDummyStreamBase::OnEvent side=" << side_ <<  " sig="
        << sig << " forwarding upward";
  ...
  StreamResult WriteData(const void* data, size_t data_len,
...
class BufferQueueStream : public BufferQueue,
                          public StreamInterface {
 public:
  BufferQueueStream(size_t capacity, size_t default_size)
      : BufferQueue(capacity, default_size) {
  }
...
class SSLStreamAdapterTestBase : public testing::Test,
 ...
 public:
  ...
  void SetUp() override {
    ...
    client_ssl_.reset(SSLStreamAdapter::Create(client_stream_));
    server_ssl_.reset(SSLStreamAdapter::Create(server_stream_));
    ...
    if (!client_cert_pem_.empty() && !client_private_key_pem_.empty()) {
      client_identity_ = SSLIdentity::FromPEMStrings(
          client_private_key_pem_, client_cert_pem_);
    } else {
      client_identity_ = SSLIdentity::Generate("client", client_key_type_);
    }
    server_identity_ = SSLIdentity::Generate("server", server_key_type_);
  ...
  void ResetIdentitiesWithValidity(int not_before, int not_after) {
    ...
    client_identity_ = SSLIdentity::GenerateForTest(client_params);
    ...
    server_identity_ = SSLIdentity::GenerateForTest(server_params);

    client_ssl_->SetIdentity(client_identity_);
    server_ssl_->SetIdentity(server_identity_);
  }

  virtual void OnEvent(StreamInterface *stream, int sig, int err) {
    ...
    if ((stream == client_ssl_.get()) && (sig & SE_WRITE)) {
      WriteData();
    }
  }
  ...
  void SetPeerIdentitiesByDigest(bool correct) {
    ...
    rv = server_identity_->certificate().ComputeDigest(DIGEST_SHA_1,
                                                       digest, 20,
                                                       &digest_len);
    ASSERT_TRUE(rv);
    if (!correct) {
    ...
    rv = client_ssl_->SetPeerCertificateDigest(DIGEST_SHA_1, digest,
                                               digest_len);
    ASSERT_TRUE(rv);


    rv = client_identity_->certificate().ComputeDigest(DIGEST_SHA_1,
    ...
    rv = server_ssl_->SetPeerCertificateDigest(DIGEST_SHA_1, digest,
                                               digest_len);
    ASSERT_TRUE(rv);
  ...
  void TestHandshake(bool expect_success = true) {
    server_ssl_->SetMode(dtls_ ? SSL_MODE_DTLS :
                         SSL_MODE_TLS);
    client_ssl_->SetMode(dtls_ ? SSL_MODE_DTLS :
                         SSL_MODE_TLS);
    ...
    if (expect_success) {
      EXPECT_TRUE_WAIT((client_ssl_->GetState() == SS_OPEN)
                       && (server_ssl_->GetState() == SS_OPEN),
                       handshake_wait_);
    } else {
      EXPECT_TRUE_WAIT(client_ssl_->GetState() == SS_CLOSED,
                       handshake_wait_);
    }
  }

  StreamResult DataWritten(SSLDummyStreamBase *from, const void *data,
  ...
  bool GetPeerCertificate(bool client, SSLCertificate** cert) {
    if (client)
 ...
 protected:
  ...
  scoped_ptr<SSLStreamAdapter> client_ssl_;
  scoped_ptr<SSLStreamAdapter> server_ssl_;
  SSLIdentity *client_identity_;  // freed by client_ssl_ destructor
  SSLIdentity *server_identity_;  // freed by server_ssl_ destructor
  int delay_;
  size_t mtu_;
  int loss_;
...
class SSLStreamAdapterTestTLS
    : public SSLStreamAdapterTestBase,
      public WithParamInterface<tuple<KeyParams, KeyParams>> {
 public:
  ...
  virtual void TestTransfer(int size) {
    ...
    EXPECT_TRUE_WAIT(server_ssl_->GetState() == SS_CLOSED, 10000);
  ...
  virtual void ReadData(StreamInterface *stream) {
    char buffer[1600];
    size_t bread;
    int err2;
    StreamResult r;
 ...
 private:
  FifoBuffer client_buffer_;
  FifoBuffer server_buffer_;
  MemoryStream send_stream_;
  MemoryStream recv_stream_;
};
 ...
 public:
  ...
  virtual void WriteData() {
    ...
    while (sent_ < count_) {
      ...
      size_t sent;
      StreamResult rv = client_ssl_->Write(packet, packet_size_, &sent, 0);
      if (rv == SR_SUCCESS) {
...
StreamResult SSLDummyStreamBase::Write(const void* data, size_t data_len,
                                              size_t* written, int* error) {
...
TEST_P(SSLStreamAdapterTestTLS, TestTLSClose) {
  TestHandshake();
  client_ssl_->Close();
  EXPECT_EQ_WAIT(SS_CLOSED, server_ssl_->GetState(), handshake_wait_);
};
...
// Disabled due to https://code.google.com/p/sm/issues/detail?id=3910
...
TEST_P(SSLStreamAdapterTestDTLS, TestDTLSSrtpHigh) {
  ...
  std::vector<int> high;
  high.push_back(SRTP_AES128_CM_SHA1_80);
  ...
  ASSERT_EQ(client_cipher, SRTP_AES128_CM_SHA1_80);
};
...
TEST_P(SSLStreamAdapterTestDTLS, TestDTLSSrtpLow) {
  MAYBE_SKIP_TEST(HaveDtlsSrtp);
  std::vector<int> low;
  low.push_back(SRTP_AES128_CM_SHA1_32);
  ...
  ASSERT_EQ(client_cipher, server_cipher);
  ASSERT_EQ(client_cipher, SRTP_AES128_CM_SHA1_32);
};
...
TEST_P(SSLStreamAdapterTestDTLS, TestDTLSSrtpMixed) {
  ...
  mixed.push_back(SRTP_AES128_CM_SHA1_80);
  mixed.push_back(SRTP_AES128_CM_SHA1_32);
...
TEST_F(SSLStreamAdapterTestDTLSFromPEMStrings, TestDTLSGetPeerCertificate) {
  ...
  SSLCertChain* client_peer_chain;
  ...
  // It must not have a chain, because the test certs are self-signed.
  SSLCertChain* server_peer_chain;
...
TEST_P(SSLStreamAdapterTestDTLS, TestGetSslCipherSuite) {
  MAYBE_SKIP_TEST(HaveDtls);
  SetupProtocolVersions(SSL_PROTOCOL_DTLS_10, SSL_PROTOCOL_DTLS_10);
  TestHandshake();
  ...
  ASSERT_EQ(client_cipher, server_cipher);
  ASSERT_EQ(
      SSLStreamAdapter::GetDefaultSslCipherForTest(
          SSL_PROTOCOL_DTLS_10, ::testing::get<1>(GetParam()).type()),
      server_cipher);
}
...
TEST_P(SSLStreamAdapterTestDTLS, TestGetSslCipherSuiteDtls12Both) {
  ...
  ASSERT_EQ(
      SSLStreamAdapter::GetDefaultSslCipherForTest(
          SSL_PROTOCOL_DTLS_12, ::testing::get<1>(GetParam()).type()),
      server_cipher);
}
...
TEST_P(SSLStreamAdapterTestDTLS, TestGetSslCipherSuiteDtls12Client) {
  MAYBE_SKIP_TEST(HaveDtls);
  SetupProtocolVersions(SSL_PROTOCOL_DTLS_10, SSL_PROTOCOL_DTLS_12);
  TestHandshake();
...
TEST_P(SSLStreamAdapterTestDTLS, TestGetSslCipherSuiteDtls12Server) {
  MAYBE_SKIP_TEST(HaveDtls);
  SetupProtocolVersions(SSL_PROTOCOL_DTLS_12, SSL_PROTOCOL_DTLS_10);
  TestHandshake();
...
INSTANTIATE_TEST_CASE_P(
    ...
    SSLStreamAdapterTestTLS,
    Combine(Values(KeyParams::RSA(1024, 65537),
                   KeyParams::RSA(1152, 65537),
                   KeyParams::ECDSA(EC_NIST_P256)),
            Values(KeyParams::RSA(1024, 65537),
                   KeyParams::RSA(1152, 65537),
                   KeyParams::ECDSA(EC_NIST_P256))));

#if !defined(MEMORY_SANITIZER)
// Fails under MemorySanitizer:
// See https://code.google.com/p/sm/issues/detail?id=5381.
INSTANTIATE_TEST_CASE_P(
