...
FileRotatingStream::FileRotatingStream(const std::string& dir_path,
    ...
    : FileRotatingStream(dir_path,
                         ...
                         num_files,
                         kWrite) {
  SM_DCHECK_GT(max_file_size, 0u);
  SM_DCHECK_GT(num_files, 1u);
}
...
FileRotatingStream::FileRotatingStream(const std::string& dir_path,
  ...
  SM_DCHECK(Filesystem::IsFolder(dir_path));
  switch (mode) {
    case kWrite: {
      file_names_.clear();
...
StreamResult FileRotatingStream::Write(const void* data,
  ...
  SM_DCHECK_LT(current_bytes_written_, max_file_size_);
  ...
  if (current_bytes_written_ >= max_file_size_) {
    SM_DCHECK_EQ(current_bytes_written_, max_file_size_);
    RotateFiles();
  }
  return result;
}
...
std::string FileRotatingStream::GetFilePath(size_t index) const {
  SM_DCHECK_LT(index, file_names_.size());
...
bool FileRotatingStream::OpenCurrentFile() {
  ...
  SM_DCHECK_LT(current_file_index_, file_names_.size());
  ...
  switch (mode_) {
    case kWrite:
      ...
      SM_DCHECK_EQ(current_file_index_, 0u);
      break;
    case kRead:
      mode = "r";
      break;
...
void FileRotatingStream::RotateFiles() {
  ...
  SM_DCHECK_LT(rotation_index_, file_names_.size());
...
std::string FileRotatingStream::GetFilePath(size_t index,
  ...
  SM_DCHECK_LE(num_digits, max_digits);
...
CallSessionFileRotatingStream::CallSessionFileRotatingStream(
    ...
    : FileRotatingStream(dir_path,
      ...
      num_rotations_(0) {
  SM_DCHECK_GE(max_total_log_size, 4u);
}

const char* CallSessionFileRotatingStream::kLogPrefix = "sm_log";
