...
void CriticalSection::Enter() EXCLUSIVE_LOCK_FUNCTION() {
  ...
  if (!recursion_count_) {
    ...
    SM_DCHECK(CurrentThreadIsOwner());
  }
  ++recursion_count_;
#endif
#endif
...
TryCritScope::~TryCritScope() {
  CS_DEBUG_CODE(SM_DCHECK(lock_was_called_));
  if (locked_)
    cs_->Leave();
}
...
void GlobalLockPod::Unlock() {
  ...
  SM_DCHECK_EQ(1, old_value) << "Unlock called without calling Lock first";
}
