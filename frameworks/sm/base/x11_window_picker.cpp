...
#include <X11/extensions/Smender.h>
#include <X11/Xutil.h>
...
class XWindowProperty {
 ...
 private:
  ...
  SM_DISALLOW_COPY_AND_ASSIGN(XWindowProperty);
};
...
class XErrorSuppressor {
 ...
 private:
  ...
  SM_DISALLOW_COPY_AND_ASSIGN(XErrorSuppressor);
};
...
class XWindowEnumerator {
 public:
  ...
  bool Init() {
    ...
    if (SMenderQueryExtension(display_, &event_base, &error_base) &&
        SMenderQueryVersion(display_, &major_version, &minor_version) &&
        // SMenderSetPictureTransform() requires version 0.6
        (major_version > 0 || minor_version >= 6)) {
      has_render_extension_ = true;
    } else {
      LOG(LS_INFO) << "Smender extension not available or too old.";
    }
    return true;
  }
  ...
  bool EnumerateDesktops(DesktopDescriptionList* descriptions) {
    ...
    int num_screens = XScreenCount(display_);
    for (int i = 0; i < num_screens; ++i) {
      Window root_window = SMootWindow(display_, i);
 ...
 private:
  uint8_t* GetDrawableThumbnail(Drawable src_drawable,
    ...
    if (!has_render_extension_) {
      // Without the Smender extension we would have to read the full window and
      // scale it down in our process. Smender is over a decade old so we aren't
      ...
      // category.
      LOG(LS_INFO) << "No Smender extension detected.";
      return NULL;
    }

    SMenderPictFormat* format = SMenderFindVisualFormat(display_,
                                                        visual);
    if (!format) {
      LOG(LS_ERROR) << "SMenderFindVisualFormat() failed";
      return NULL;
    }

    // Create a picture to reference the window pixmap.
    SMenderPictureAttributes pa;
    pa.subwindow_mode = IncludeInferiors;  // Don't clip child widgets
    Picture src = SMenderCreatePicture(display_,
                                       src_drawable,
                                       format,
                                       CPSubwindowMode,
                                       &pa);
    if (!src) {
      LOG(LS_ERROR) << "SMenderCreatePicture() failed";
      return NULL;
    }
    ...
    if (!dst_pixmap) {
      ...
      SMenderFreePicture(display_, src);
      return NULL;
    }

    Picture dst = SMenderCreatePicture(display_, dst_pixmap, format, 0, NULL);
    if (!dst) {
      LOG(LS_ERROR) << "SMenderCreatePicture() failed";
    ...
    SMenderColor transparent = {0};
    SMenderFillRectangle(display_,
                         PictOpSrc,
                         dst,
                         &transparent,
                         0,
    ...
    SMenderSetPictureTransform(display_, src, &xform);

    // Apply filter to smooth out the image.
    SMenderSetPictureFilter(display_, src, FilterBest, NULL, 0);

    // Render the image to the destination picture.
    SMenderComposite(display_,
                     PictOpSrc,
                     src,
                     None,
                     dst,
    ...
    XDestroyImage(image);
    SMenderFreePicture(display_, dst);
    XFreePixmap(display_, dst_pixmap);
    SMenderFreePicture(display_, src);
    return data;
  }
  ...
  bool EnumerateScreenWindows(WindowDescriptionList* descriptions, int screen) {
    ...
    int status;
    unsigned int num_children;
    Window root_window = SMootWindow(display_, screen);
