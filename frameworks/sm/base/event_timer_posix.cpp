...
bool EventTimerPosix::StartTimer(bool periodic, unsigned long time) {
  ...
  const char* thread_name = "Sm_event_timer_thread";
  timer_thread_.reset(new PlatformThread(Run, this, thread_name));
  periodic_ = periodic;
  time_ = time;
  timer_thread_->Start();
