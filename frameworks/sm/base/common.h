...
#ifndef SM_BASE_COMMON_H_
#define SM_BASE_COMMON_H_
...
#ifndef SM_UNUSED
#define SM_UNUSED(x) SmUnused(static_cast<const void*>(&x))
#define SM_UNUSED2(x, y) SmUnused(static_cast<const void*>(&x)); \
    SmUnused(static_cast<const void*>(&y))
#define SM_UNUSED3(x, y, z) SmUnused(static_cast<const void*>(&x)); \
    SmUnused(static_cast<const void*>(&y)); \
    SmUnused(static_cast<const void*>(&z))
#define SM_UNUSED4(x, y, z, a) SmUnused(static_cast<const void*>(&x)); \
    SmUnused(static_cast<const void*>(&y)); \
    SmUnused(static_cast<const void*>(&z)); \
    SmUnused(static_cast<const void*>(&a))
#define SM_UNUSED5(x, y, z, a, b) SmUnused(static_cast<const void*>(&x)); \
    SmUnused(static_cast<const void*>(&y)); \
    SmUnused(static_cast<const void*>(&z)); \
    SmUnused(static_cast<const void*>(&a)); \
    SmUnused(static_cast<const void*>(&b))
inline void SmUnused(const void*) {}
#endif  // SM_UNUSED
...
// calls up the stack.
#define ASSERT(x) \
    (AssertNoBreak((x), __FUNCTION__, __FILE__, __LINE__, #x) ? \
     (void)(1) : __debugbreak())
#else
#define ASSERT(x) \
...
#endif  // SM_BASE_COMMON_H_    // NOLINT
