...
#ifndef SM_BASE_FAKE_SSL_IDENTITY_H_
#define SM_BASE_FAKE_SSL_IDENTITY_H_
...
class FakeSSLCertificate : public SSLCertificate {
 public:
  ...
  virtual bool ComputeDigest(const std::string& algorithm,
                             ...
                             size_t size,
                             size_t* length) const {
    *length = ComputeDigest(algorithm, data_.c_str(), data_.size(),
                                       digest, size);
    return (*length != 0);
  }
...
#endif  // SM_BASE_FAKE_SSL_IDENTITY_H_
