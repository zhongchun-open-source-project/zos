...
class SSLIdentityTest : public testing::Test {
 public:
  ...
  virtual void SetUp() {
    identity_rsa1_.reset(SSLIdentity::Generate("test1", KT_RSA));
    identity_rsa2_.reset(SSLIdentity::Generate("test2", KT_RSA));
    identity_ecdsa1_.reset(SSLIdentity::Generate("test3", KT_ECDSA));
    identity_ecdsa2_.reset(SSLIdentity::Generate("test4", KT_ECDSA));
    ...
    test_cert_.reset(SSLCertificate::FromPEMString(kTestCertificate));
 ...
 private:
  scoped_ptr<SSLIdentity> identity_rsa1_;
  scoped_ptr<SSLIdentity> identity_rsa2_;
  scoped_ptr<SSLIdentity> identity_ecdsa1_;
  scoped_ptr<SSLIdentity> identity_ecdsa2_;
  scoped_ptr<SSLCertificate> test_cert_;
};

TEST_F(SSLIdentityTest, FixedDigestSHA1) {
  TestDigestForFixedCert(DIGEST_SHA_1, 20, kTestCertSha1);
}

// HASH_AlgSHA224 is not supported in the chromium linux build.
TEST_F(SSLIdentityTest, FixedDigestSHA224) {
  TestDigestForFixedCert(DIGEST_SHA_224, 28, kTestCertSha224);
}

TEST_F(SSLIdentityTest, FixedDigestSHA256) {
  TestDigestForFixedCert(DIGEST_SHA_256, 32, kTestCertSha256);
}

TEST_F(SSLIdentityTest, FixedDigestSHA384) {
  TestDigestForFixedCert(DIGEST_SHA_384, 48, kTestCertSha384);
}

TEST_F(SSLIdentityTest, FixedDigestSHA512) {
  TestDigestForFixedCert(DIGEST_SHA_512, 64, kTestCertSha512);
}

// HASH_AlgSHA224 is not supported in the chromium linux build.
TEST_F(SSLIdentityTest, DigestSHA224) {
  TestDigestForGeneratedCert(DIGEST_SHA_224, 28);
}

TEST_F(SSLIdentityTest, DigestSHA256) {
  TestDigestForGeneratedCert(DIGEST_SHA_256, 32);
}

TEST_F(SSLIdentityTest, DigestSHA384) {
  TestDigestForGeneratedCert(DIGEST_SHA_384, 48);
}

TEST_F(SSLIdentityTest, DigestSHA512) {
  TestDigestForGeneratedCert(DIGEST_SHA_512, 64);
}
...
TEST_F(SSLIdentityTest, FromPEMStringsEC) {
  ...
  static const char kCERT_PEM[] =
      ...
      "AkFVMRMwEQYDVQQIDApTb21lLVN0YSMlMSEwHwYDVQQKDBhJbnRlcm5ldCBXaWRn\n"
      "aSMzIFB0eSBMdGQwIBcNMTUwNjMwMTMwMTIyWhgPMjI4OTA0MTMxMzAxMjJaMEUx\n"
      "CzAJBgNVBAYTAkFVMRMwEQYDVQQIDApTb21lLVN0YSMlMSEwHwYDVQQKDBhJbnRl\n"
      "cm5ldCBXaWRnaSMzIFB0eSBMdGQwWTATBgcqhkjOPQIBBggqhkjOPQMBBwNCAAQM\n"
 ...
 public:
  ...
  void TestExpireTime(int times) {
    for (int i = 0; i < times; i++) {
      ...
      params.not_after = CreateRandomId() % 0x80000000;
