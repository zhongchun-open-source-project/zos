...
#ifdef _WIN32
#include "sm/base/trace_win.h"
#else
#include "sm/base/trace_posix.h"
#endif  // _WIN32
...
int32_t TraceImpl::AddThreadId(char* trace_message) const {
  uint32_t thread_id = CurrentThreadId();
...
int32_t TraceImpl::AddMessage(
  ...
  int length = 0;
  if (written_so_far >= SM_TRACE_MAX_MESSAGE_SIZE) {
    return -1;
  }
  // - 2 to leave room for newline and NULL termination.
#ifdef _WIN32
  length = _snprintf(trace_message,
                     SM_TRACE_MAX_MESSAGE_SIZE - written_so_far - 2,
                     "%s", msg);
  if (length < 0) {
    length = SM_TRACE_MAX_MESSAGE_SIZE - written_so_far - 2;
...
void TraceImpl::WriteToFile(const char* msg, uint16_t length) {
  ...
  if (row_count_text_ > SM_TRACE_MAX_FILE_SIZE) {
    // wrap file
    row_count_text_ = 0;
    trace_file_->Flush();
  ...
  if (row_count_text_ == 0) {
    char message[SM_TRACE_MAX_MESSAGE_SIZE + 1];
  ...
  char trace_message[SM_TRACE_MAX_MESSAGE_SIZE];
...
void Trace::set_level_filter(int filter) {
  AtomicOps::ReleaseStore(&level_filter_, filter);
}

// static
int Trace::level_filter() {
  return AtomicOps::AcquireLoad(&level_filter_);
}
...
void Trace::Add(const TraceLevel level, const TraceModule module,
  ...
  if (trace) {
    if (trace->TraceCheck(level)) {
      char temp_buff[SM_TRACE_MAX_MESSAGE_SIZE];
      char* buff = 0;
      if (msg) {
        va_list args;
        va_start(args, msg);
#ifdef _WIN32
        _vsnprintf(temp_buff, SM_TRACE_MAX_MESSAGE_SIZE - 1, msg, args);
#else
        vsnprintf(temp_buff, SM_TRACE_MAX_MESSAGE_SIZE - 1, msg, args);
#endif
        va_end(args);
        buff = temp_buff;
      }
