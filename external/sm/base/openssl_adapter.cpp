...
#include "sm/base/openssl_adapter.h"
...
#include "sm/base/win32.h"  // NOLINT
...
#include "sm/base/openssl.h"
#include "sm/base/safe_conversions.h"
#include "sm/base/ssl_roots.h"
...
static BIO* BIO_new_socket(AsyncSocket* socket) {
...
static int socket_read(BIO* b, char* out, int outl) {
  if (!out)
    return -1;
  AsyncSocket* socket = static_cast<AsyncSocket*>(b->ptr);
...
static int socket_puts(BIO* b, const char* str) {
  return socket_write(b, str, checked_cast<int>(strlen(str)));
}
...
bool OpenSSLAdapter::InitializeSSL(VerificationCallback callback) {
  if (!InitializeSSLThread() || !SSL_library_init())
      return false;
#if !defined(ADDRESS_SANITIZER) || !defined(SM_MAC) || defined(SM_IOS)
