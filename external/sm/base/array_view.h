...
#ifndef SM_BASE_ARRAY_VIEW_H_
#define SM_BASE_ARRAY_VIEW_H_
...
class ArrayView final {
 public:
  ...
  // Buffer to ArrayView<uint8_t> (with the same const behavior as
  // std::vector).
  template <typename U>
  ...
  T& operator[](size_t idx) const {
    SM_DCHECK_LT(idx, size_);
    SM_DCHECK(data_);  // Follows from size_ > idx and the class invariant.
    return data_[idx];
  }
 ...
 private:
  ...
  void CheckInvariant() const { SM_DCHECK_EQ(!data_, size_ == 0); }
  T* data_;
  size_t size_;
};
