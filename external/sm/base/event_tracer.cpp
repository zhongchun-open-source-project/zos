...
#include "sm/base/event_tracer.h"
...
class EventLogger final {
 public:
  ...
  ~EventLogger() { SM_DCHECK(thread_checker_.CalledOnValidThread()); }
...
// https://docs.google.com/document/d/1CvAClvFfyA5R-PhYUmn5OOQtYMH4h6I0nSsKchNAySU/preview
  ...
  void Start(FILE* file, bool owned) {
    ...
    SM_CHECK_EQ(0,
                 AtomicOps::CompareAndSwap(&g_event_logging_active, 0, 1));
  ...
  void Stop() {
    ...
    if (AtomicOps::CompareAndSwap(&g_event_logging_active, 1, 0) == 0)
      return;
...
void InternalAddTraceEvent(char phase,
  ...
  if (AtomicOps::AcquireLoad(&g_event_logging_active) == 0)
    return;
...
void SetupInternalTracer() {
  SM_CHECK(AtomicOps::CompareAndSwapPtr(
...
void ShutdownInternalTracer() {
  StopInternalCapture();
  EventLogger* old_logger = AtomicOps::AcquireLoadPtr(&g_event_logger);
  SM_DCHECK(old_logger);
  SM_CHECK(AtomicOps::CompareAndSwapPtr(
