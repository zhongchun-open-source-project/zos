...
#include "sm/base/file_utils_mock.h"
...
bool GetProxyInfo(const std::string prefs, ProxyInfo* info) {
  std::vector<FakeFileSystem::File> files;
  files.push_back(FakeFileSystem::File("profiles.ini",
                                                  kFirefoxProfilesIni));
  files.push_back(FakeFileSystem::File("prefs.js", prefs));
  FilesystemScope fs(new FirefoxPrefsFileSystem(files));
