...
#include "sm/base/scopedptr_collection.h"
...
TEST(AtomicOpsTest, SimplePtr) {
  ...
  EXPECT_TRUE(AtomicOps::AcquireLoadPtr(&foo) == nullptr);
  // Setting using compare and swap should work.
  EXPECT_TRUE(AtomicOps::CompareAndSwapPtr(
                  &foo, static_cast<Foo*>(nullptr), a.get()) == nullptr);
  EXPECT_TRUE(AtomicOps::AcquireLoadPtr(&foo) == a.get());
  ...
  EXPECT_TRUE(AtomicOps::CompareAndSwapPtr(&foo, a.get(), b.get()) ==
              a.get());
  EXPECT_TRUE(AtomicOps::AcquireLoadPtr(&foo) == b.get());
}
