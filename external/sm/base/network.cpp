...
#include "sm/base/socket.h"  // includes something that makes windows happy
...
std::string AdapterTypeToString(AdapterType type) {
  switch (type) {
    ...
    case ADAPTER_TYPE_LOOPBACK:
      return "Loopback";
    default:
      SM_DCHECK(false) << "Invalid type " << type;
      return std::string();
  }
}
...
void NetworkManagerBase::GetAnyAddressNetworks(NetworkList* networks) {
  if (!ipv4_any_address_network_) {
    ...
    ipv4_any_address_network_.reset(
        new Network("any", "any", ipv4_any_address, 0));
  ...
  if (ipv6_enabled()) {
    if (!ipv6_any_address_network_) {
      ...
      ipv6_any_address_network_.reset(
          new Network("any", "any", ipv6_any_address, 0));
...
bool BasicNetworkManager::CreateNetworks(bool include_ignored,
  ...
  scoped_ptr<IfAddrsConverter> ifaddrs_converter(CreateIfAddrsConverter());
