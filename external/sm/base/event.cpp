...
Event::Event(bool manual_reset, bool initially_signaled)
  ...
  SM_CHECK(pthread_mutex_init(&event_mutex_, NULL) == 0);
  SM_CHECK(pthread_cond_init(&event_cond_, NULL) == 0);
}
...
bool Event::Wait(int milliseconds) {
  ...
  // consistent with auto-reset events in SM_WIN
