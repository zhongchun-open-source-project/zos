...
class DBusSigFilterTest : public DBusSigFilter {
 ...
 private:
  static std::string GetFilter() {
    return DBusSigFilter::BuildFilterString("", "", SIG_NAME);
  }
...
TEST(DBusMonitorTest, StartStopStartStop) {
  ...
  scoped_ptr<DBusMonitor> monitor;
  monitor.reset(DBusMonitor::Create(DBUS_BUS_SYSTEM));
  if (monitor) {
...
TEST(DBusMonitorTest, ConcurrentMonitors) {
  ...
  scoped_ptr<DBusMonitor> monitor1;
  monitor1.reset(DBusMonitor::Create(DBUS_BUS_SYSTEM));
  if (monitor1) {
    EXPECT_TRUE(monitor1->AddFilter(&filter1));
    DBusSigFilterTest filter2;
    scoped_ptr<DBusMonitor> monitor2;
    monitor2.reset(DBusMonitor::Create(DBUS_BUS_SYSTEM));
