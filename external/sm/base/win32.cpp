...
const char* inet_ntop_v6(const void* src, char* dst, socklen_t size) {
  ...
  if (runpos[0] == 1 && (maxpos == 5 ||
    ...
    *cursor++ = ':';
    if (maxpos == 4) {
      cursor += sprintfn(cursor, INET6_ADDRSTRLEN - 2, "ffff:");
    }
...
bool Utf8ToWin32Filename(const std::string& utf8, std::wstring* filename) {
