...
#ifndef SM_BASE_TEST_UTILS_H_
#define SM_BASE_TEST_UTILS_H_

// Utilities for testing sm infrastructure in unittests

#if defined(SM_LINUX) && !defined(SM_ANDROID)
#include <X11/Xlib.h>
#include <X11/extensions/Smandr.h>
...
inline Pathname GetSiblingDirectory(
    const std::string& parallel_dir) {
  Pathname path = Filesystem::GetCurrentDirectory();
  while (!path.empty()) {
    Pathname potential_parallel_dir = path;
    potential_parallel_dir.AppendFolder(parallel_dir);
    if (Filesystem::IsFolder(potential_parallel_dir)) {
...
inline Pathname GetGoogle3Directory() {
  return GetSiblingDirectory("google3");
}

inline Pathname GetTalkDirectory() {
...
inline bool IsScreencastingAvailable() {
  ...
  if (!SMRQueryExtension(display, &ignored_int, &ignored_int) ||
      !SMRQueryVersion(display, &major_version, &minor_version) ||
      major_version < 1 ||
      (major_version < 2 && minor_version < 3)) {
    LOG(LS_WARNING) << "SMandr version: " << major_version << "."
                    << minor_version;
    LOG(LS_WARNING) << "SMandr is not supported or is too old (pre 1.3).";
    return false;
  }
#endif
  return true;
